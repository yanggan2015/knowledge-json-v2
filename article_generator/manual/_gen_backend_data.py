...
def _generate_module(domain: str, module: str) -> dict:
    ...
    core = facts.get("core", f"{module} 是 {domain} 的核心能力模块，连接业务需求与底层运行时。")
    ...
    return {
        ...
        "workflow": workflow_extra or (
            f"1. 阅读 {domain} 官方 {module} 文档与示例\n"
            f"2. 在本地/开发环境最小配置验证\n"
            f"3. 集成到主流程并补充单元/集成测试\n"
            f"4. 配置监控告警与 runbook\n"
            f"5. 灰度发布并观察核心指标"
        ),
