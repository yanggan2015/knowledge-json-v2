...
**10 个主题包**（同模块多篇 chapter）→ **合并为 10 篇完整长文** → 发布 CSDN → 登记表 → `git push`。

> 不再「选多篇短 chapter 各发一篇」；而是保证每个知识点完整、篇幅足够。  
> 若 CSDN 免费日额度不足 8 篇，超出部分会自动存**草稿**（登记表照常追加）。
...
| 合并 | 2～6 篇源 md → 1 篇约 **1000 行干货** `articles/csdn-merged/*.md` |
| 图示 | 合并长文 **≥2 处 Mermaid**（调用链 + 架构/数据流） |
| 发布 | 每天 **10 篇**（`DAILY_PUBLISH_COUNT`，以平台实测上限为准） |
...
# 临时改篇数：
# DAILY_PUBLISH_COUNT=8 bash ~/knowledge-json-v2/scripts/daily-csdn/run.sh
```

## 注意

- CSDN 依赖本机浏览器登录态；`~/.config/csdn-daily.env` 配置 `DISPLAY`。
- 今日正式发文额度用尽时会存草稿；登记表仍追加源路径，避免重复合并。
- 也可在 `~/.config/csdn-daily.env` 写入 `DAILY_PUBLISH_COUNT=8` 固化篇数。
