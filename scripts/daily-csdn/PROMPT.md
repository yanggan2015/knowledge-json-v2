# 每日任务：主题合并 → 产出 8 篇完整长文 → 发布 CSDN → 登记 → git push

你是本仓库的自动化 Agent。请**严格按序**完成下列全部步骤，不要只规划不执行。

## 核心策略（必读）

**每日目标：发布 8 篇合并长文。** 不要直接把 8 个零散 chapter 各发一篇——它们往往篇幅过小、知识点割裂。
...
3. 每天产出并发布 **恰好 8 篇**这样的合并长文（由 `DAILY_PUBLISH_COUNT` 控制，默认 8）。
4. 合并后的成稿写入 `articles/csdn-merged/<系列>-<主题>.md`（单文件对应 CSDN 一篇）。
5. **额度说明**：若 CSDN 提示「已达今日发文上限」，继续对剩余篇执行发布脚本——脚本会自动存**草稿**；草稿也要登记，避免明天重复推。不要因上限而跳过合并或跳过 `publish.sh`。
...
3. 选出 **8 个**未发布过的主题包（数量以 `DAILY_PUBLISH_COUNT` 为准，默认 8）。优先级：
...
5. 遇到发文上限：**不要停止**，继续发完 8 篇（后续一般为草稿）。
