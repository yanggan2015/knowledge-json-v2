...
**篇幅与质量**：合并文建议 **2500～6000 字**（信息密度优先，不注水）；必须含 **源码锚点 / 调用链 / 重点知识**；**有必要处用 Mermaid**；**禁止**文末 Checklist/自检清单等与知识无关的模板段；对照 `QUALITY.md`。
...
3. 选出 **10 个**未发布过的主题包（数量以 `DAILY_PUBLISH_COUNT` 为准，默认 10）。优先级：
