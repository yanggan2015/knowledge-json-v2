...
> **领域**：C++ ｜ **模块**：Lambda与函数对象 ｜ **难度**：高级 ｜ **类型**：性能优化


## 导读

本章系统讲解 **C++** 中 **Lambda与函数对象** 的相关知识（性能优化）。本章从度量指标、瓶颈定位与优化手段三方面讲解 **Lambda与函数对象** 的性能议题。内容基于主流框架与工程实践撰写，不依赖过时概念堆砌。
...
针对 **Lambda与函数对象**，性能工作应「先度量后优化」：明确 P50/P95/P99 与资源占用基线，用 profiler/trace 定位热点，优先处理 I/O、锁竞争与算法复杂度问题，避免无数据支撑的微调。
