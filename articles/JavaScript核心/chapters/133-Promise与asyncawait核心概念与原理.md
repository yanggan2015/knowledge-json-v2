...
> **领域**：JavaScript核心 ｜ **模块**：Promise与async/await ｜ **难度**：高级 ｜ **类型**：基础概念
...
### 为什么需要 Promise与async/await

工程实践中，Promise与async/await 解决的是「在 ES2024 约束下，如何可靠、可维护地完成特定职责」的问题。初学者常只记 API 而不理解动机——场景变化时便无法判断该不该用、怎么用。

### 核心原理

Promise与async/await 的设计遵循：**职责单一**、**接口稳定**、**可观测**。在 **高级** 阶段，重点是把这三条映射到具体概念与操作上。
...
- **理解Promise与async/await的设计思想与适用场景**：落地时需明确验收标准与回滚方案。
- **掌握Promise与async/await的核心API与配置项**：落地时需明确验收标准与回滚方案。
...
- 说清 **Promise与async/await核心概念与原理** 在 JavaScript核心/Promise与async/await 中的定位。
...
---
*章节 ID: 133 ｜ 领域: JavaScript核心 ｜ 版本: 2.0*