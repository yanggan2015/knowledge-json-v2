...
> **分类**：系统底层 ｜ **技术生态**：NASM/GAS, objdump, GDB, Compiler Explorer, ARM Keil/GCC


## 领域定位

汇编语言是机器指令的人类可读形式，直接操控寄存器、栈与内存。x86_64 与 ARM 汇编是逆向工程、内核启动、性能关键路径与嵌入式开发的必备技能。

面向底层开发者与安全研究员，强调调用约定、寻址方式与 ABI 兼容性。

本领域常用技术栈与工具包括：NASM/GAS, objdump, GDB, Compiler Explorer, ARM Keil/GCC。
