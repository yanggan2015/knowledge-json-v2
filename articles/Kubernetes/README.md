...
> **分类**：DevOps ｜ **技术生态**：Helm、Prometheus、Istio、CNI（Calico/Cilium）


## 领域定位

Kubernetes 以声明式 API 管理容器化 workload，Pod 共享网络命名空间，控制器 reconcile 期望状态，是云原生编排事实标准。

覆盖 Pod、Deployment、Service、Ingress、存储、RBAC 与 Helm，面向平台与 SRE。

本领域常用技术栈与工具包括：Helm、Prometheus、Istio、CNI（Calico/Cilium）。
