...
> **分类**：数据存储 ｜ **技术生态**：InnoDB、ProxySQL、Percona Toolkit、Orchestrator


## 领域定位

MySQL 8 默认 InnoDB，支持窗口函数、CTE 与 JSON。生产环境关注主从复制、读写分离、分库分表与慢查询优化。

从架构、InnoDB B+树、执行计划到高可用运维，面向 DBA 与后端工程师。

本领域常用技术栈与工具包括：InnoDB、ProxySQL、Percona Toolkit、Orchestrator。
