# 原理剖析：系统调用与IPC在Linux内核中的应用

> **领域**：Linux内核 ｜ **模块**：系统调用与IPC ｜ **难度**：实战 ｜ **类型**：底层原理


## 导读

本章系统讲解 **Linux内核** 中 **系统调用与IPC** 的相关知识（底层原理）。本章从底层原理出发拆解 **系统调用与IPC** 与运行时、内核或协议栈的交互。内容基于主流框架与工程实践撰写，不依赖过时概念堆砌。
