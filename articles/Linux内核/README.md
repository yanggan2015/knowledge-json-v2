...
> **分类**：系统底层 ｜ **技术生态**：Linux 6.x, GRUB/systemd, perf, ftrace, bpftrace, BCC, crash, kdump, LKML


## 领域定位

Linux 内核是开源操作系统核心，管理 CPU、内存、设备与网络，向上提供系统调用与 POSIX 语义。理解启动链、调度器、虚拟内存与驱动模型，是内核开发、性能调优与故障排查的基础。

面向内核开发者、系统程序员与 SRE，以 Linux 6.x 主线为参照，结合源码与观测工具讲解机制。

本领域常用技术栈与工具包括：Linux 6.x, GRUB/systemd, perf, ftrace, bpftrace, BCC, crash, kdump, LKML。
