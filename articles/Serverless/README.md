...
> **分类**：后端开发 ｜ **技术生态**：AWS Lambda、API Gateway、阿里云函数计算、Knative


## 领域定位

Serverless 将运维抽象至云厂商，开发者以函数为单位按 invocation 计费。事件驱动、自动扩缩与冷启动是其核心特征。

覆盖 FaaS、BFF、Step Functions 编排、冷启动优化与成本模型，适合事件型与流量波动大的 workload。

本领域常用技术栈与工具包括：AWS Lambda、API Gateway、阿里云函数计算、Knative。
