...
- [ ] `driver.of_match_table` 与 DTS `compatible` **逐字符一致**，且有 `MODULE_DEVICE_TABLE(of, …)`
- [ ] 能口述：`platform_driver_register` → `platform_match` → `of_match_device` → `really_probe` → `probe`
- [ ] probe 内资源用 `devm_*`；GPIO 用 `gpiod_get`，MMIO 批量读写考虑 `devm_regmap_init_mmio`
- [ ] 依赖未就绪返回 `-EPROBE_DEFER`，不用 `msleep`；会查 `devices_deferred`
- [ ] 字符设备：`alloc_chrdev_region` → `cdev_add` → `device_create`；`/proc/devices` 与 `/dev` 主次一致
- [ ] 能解释 `chrdev_open` 查 `cdev_map` 再 `replace_fops` 到驱动 `file_operations`
- [ ] remove/错误路径对称释放；`rmmod` 前确认无 `lsof` 占用
- [ ] 排障顺序固定：platform 设备 → driver 链接 → `/dev` 节点 → `strace`/`dmesg`
