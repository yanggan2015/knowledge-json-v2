...
成稿前建议对照下列项自测一遍，再提交评审或发布；现场排障可打印本节贴机柜侧。
- [ ] 能画出 MCU 路径：`tcpip_thread` ← `tcpip_input` ← 驱动 RX；TX：`tcp_write` → `linkoutput` → MAC
- [ ] 能画出 Linux 路径：`dev_queue_xmit` → `ndo_start_xmit`；RX：`napi_schedule` → `napi_poll` → `netif_receive_skb`
- [ ] 能说明 PHY link up 与 L3 可达的区别，并区分 carrier / ARP / 路由问题
- [ ] 配置 IP 时明确 LwIP `dhcp.c` 与 Linux `udhcpc` 流程，顺序为 up → DHCP/静态
- [ ] checksum offload 会用 `ethtool -k` 做对比实验；MTU 用 `ping -M do -s` 验证
- [ ] 零拷贝场景能说清 pbuf/skb 释放时机，避免 DMA 期间释放缓冲
- [ ] 已对照真实目录：`src/core/tcpip.c`（MCU）与 `net/core/dev.c`、`drivers/net/ethernet/`（Linux）
- [ ] Bring-up 顺序固定：PHY/MDIO → MAC/DMA → netif up → 静态 IP 验证 → 再开 DHCP
- [ ] 性能调优前已有 iperf/丢包/CPU 基线，改 ring/offload 可对比回归
