...
- [ ] 控制器与子设备 `compatible` 与驱动 `of_match_table` **逐字符**一致
- [ ] I2C 子节点 `reg` 为 **7-bit 地址**；SPI 子节点 `reg` 为 **片选号**
- [ ] `clock-frequency` / `spi-max-frequency` 与硬件、走线能力匹配
- [ ] I2C 原理图确认 **外部上拉**；开漏 GPIO 使用 `GPIOD_OPEN_DRAIN`
- [ ] `pinctrl-0` 已将引脚切到 peripheral 功能，非 GPIO 默认态
- [ ] `clocks` / `clock-names` 与驱动 `devm_clk_get` 名字一致且已 enable
- [ ] 用户态 GPIO 用 **libgpiod + `/dev/gpiochipN`**，不用 sysfs export
- [ ] 能口述 **`gpiod_get` → gpiolib**、**`i2c_transfer` → master_xfer**、**`spi_sync` → transfer_one_message**、**uart probe → ttyS** 四条链
- [ ] DMA 问题时对比 PIO 路径，查 buffer 映射与 `dma-names`
- [ ] 依赖未就绪返回 `-EPROBE_DEFER`，不用 `msleep` 硬等
- [ ] **`pinmux-pins` / `clk_summary`** 已对照，确认 AF 复用与外设时钟 enable
- [ ] RS485 板用 **`linux,rs485-*` 或 TIOCSRS485** 验证 DE 时序
- [ ] DMA 异常时用 **`use_dma=0` 或关 DMA** 对比，查 cache sync
- [ ] 会用 **`i2ctransfer` / `gpio-event` / `gpiofind`** 做最小物理层隔离
- [ ] probe 失败时按 **第 11 节八阶段表** 逐级排除，不跳步改业务驱动
