...
- [ ] 能指出 `submit_bio` → `blk_mq_submit_bio` → `queue_rq` → `complete` 的文件级路径
- [ ] `ls /sys/block/<dev>/` 存在，`size` 与驱动上报容量一致
- [ ] `queue_rq` 与完成/超时回调成对；刺激 I/O 后无永久 D 状态读进程
- [ ] `logical_block_size` / `max_sectors` 与硬件或仿真后端匹配
- [ ] `iostat` / `blktrace` 能看到请求下发与完成，而非只在 FS 层空转
- [ ] 用 `dd … oflag=direct` 或 fio direct 验证过驱动路径
- [ ] 卸载路径先停 I/O 再 `del_gendisk`，无 UAF
- [ ] 未在 `queue_rq` 中长时间阻塞或睡眠
